...
def build_query(
    ...
    SELECT
        ...
        as_number, as_name, avg_upload_speed_mbps, avg_latency_ms, num_tests,
...
def get_upload_data(request) -> Any:
    """
    Handles HTTP requests for uploading network measurement data.
    Performs input validation, constructs BigQuery queries, and returns paginated results.
    """

    # CORS headers to allow cross-origin requests (e.g., from web browsers)
    headers = {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type",
    ...
    if year is not None and (year < 2019 or year > 2100):  # Example year range
